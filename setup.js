...
;(async () => {
  ...
  console.log(
    'Welcome to the BluBot setup!\nThis script will set up your bot with its token, guild, channels and roles.\nPress Enter to continue...'
  )
  await readline()
  console.log('Please enter your Discord bot token:')
  current.token = await readline()
  console.log(
    'Now enter the guild ID of your server. You can find this by enabling Developer Mode and right-clicking your guild.'
  )
  current.guildId = await readline()
  console.log(
    "That's a really nice server you got! Just kidding, i'm not sentient ;(\nWould you like to customize your bot with custom colors for embeds? [y/N]"
  )
  const customColors = (await readline()).toLowerCase()
  if (customColors === 'y') {
    console.log(
      'Alright. Please enter a HEX color you would like as an accent color. This will apply to generic embeds. Leave blank for default.'
    )
    current.customization.accent = await readline()
    console.log(
      'Now enter a HEX color you would like as a "good" color. This will apply to embeds which remove a moderation. Leave blank for default.'
    )
    ...
    console.log(
      'Now enter a HEX color you would like as a "bad" color. This will apply to embeds which add a fatal moderation like a ban. Leave blank for default.'
    )
...
Does this look correct? [Y/n]
  ...
  console.log(
    "Success! Install dependencies with the command 'npm install', and start the bot with 'npm start'!"
  )
  process.exit()
})()
